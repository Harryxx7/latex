...
\usepackage{float} % exact figure placement [H]
...
预期某新项目未来10年，每年售出6,000单位产品，每单位带来50美元的净现金流。项目 的必要报酬率（折现率 $r _ { s } )$ ）为16%，初始投资为 120万美元。 
...
\begin{tabularx}{\textwidth}{llYY}
  ...
  初创与转型期 & 两阶段增长模型 & 增长率分段变化,前高后低。初期不规则高增长,后期转入永续平稳增长阶段。 & (其中 $D_1 = D_0(1 + g)$)$P_0 = \sum_{t=1}^{n} \dfrac{D_t}{(1 + r_s)^t} + \dfrac{V_n}{(1 + r_s)^n}$(其中 $V_n = \dfrac{D_{n+1}}{r_s - g_2}$) \\
  \bottomrule
\end{tabularx}
\end{table}
...
设公司未来各期预期分红以固定增长率 g 增长，即 $D _ { t } = D _ { 1 } ( 1 + g ) ^ { t - 1 }$ 。若投资者要求的股 权必要报酬率为 $r _ { s }$ （且满足收敛条件 $r _ { s } > g )$ ），将各期红利代入绝对估值DDM 模型： 
...
$$
\boxed {V _ {0} = \frac {D _ {1}}{r _ {s} - g}} \quad \text {(或写为:} \boxed {V _ {0} = \frac {D _ {0} (1 + g)}{r _ {s} - g}}
$$
...
$\bullet \ t = 1 : D _ { 1 } = D _ { 0 } ( 1 + g _ { 1 } ) = 1 . 2 5$ 美元，折现至 t = 0 为 1.251.2 
...
$$
\boxed {V _ {0} = \underbrace {\frac {E _ {1}}{r _ {s}}} _ {\text {现金牛价值}} + \mathbf {N P V G O}}
$$
...
2. 维持负债结构，总资产同比例扩张： $\Delta A = \Delta S \times \mathrm { E M } _ { \circ }$ 。 

3. 新增资产按周转效率创造销售额： $\Delta \mathrm { S a l e s } = \Delta A \times \Delta \mathrm { T O } .$ 。 

4. 新增收入按净利润率转化为净利润： $\Delta \mathbf { N I } _ { 1 } = \Delta \mathbf { S } \mathrm { a l e s } \times \mathbf { P M } .$ 。 

5. 两侧相乘化简，利用杜邦分解式 $\mathbf { E M } \times \mathbf { A T O } \times \mathbf { P M } = \mathbf { R O E }$ 可得： 

$$
g = \frac {\Delta \mathbf {N I} _ {1}}{\mathbf {N I} _ {0}} = \mathbf {R O E} \times b
$$
...
$$
\boxed {\mathbf {P E} = \frac {P}{\mathbf {E P S}}, \quad \text {其倒数为盈利收益率:} \frac {1}{\mathbf {P E}} = \frac {\mathbf {E P S}}{P}}
$$
...
注. 案例背景：某公司股票当前 $\mathrm { E P S } _ { 0 } = 1 . 0 0 \ : \vec { \mathcal { \pi } }$ ，当前市价 $P _ { 0 } = 1 0 . 0 0 \stackrel { \_ } { \_ }$ 。当期股利支付率 $d = 5 0 \%$ ，再 投资回报率 $\mathrm { R O E } = 1 0 \%$ 。必要报酬率 $r _ { s } = 1 0 . 2 5 \%$ 。若公司决定降低分红，将股利支付率下调至 30%（即 留存比率 $b'$ 提高为70%）： 
...
2. 内在联系：根据内在公式可拆解为： $\mathbf { P B } = \mathbf { P E } \times \mathbf { R O E }$ 。这说明市净率的高低最终依旧取决 于资产能够创造盈余的效率（即ROE的水平）。 

案例 5 (2024  上财：理论市盈率与投资决策). 某公司股票当前价格是 20 元，最近一期 $\mathrm { { E P S } _ { 0 } = }$ 0.40 元，每股股利 $D _ { 0 } = 0 . 3 2$ 元。预计以后每股收益按每年 g = 5% 增长，期望必要报酬率为 $r _ { s } = 1 0 \%$ 。求：（1）该公司股票的理论市盈率；（2）结合实际市盈率给出投资决策建议。 

\subsubsection{【答案/解析】}

1. 计算下一期预期红利： $D _ { 1 } = D _ { 0 } \times ( 1 + g ) = 0 . 3 2 \times 1 . 0 5 = 0 . 3 3 6 ~ \overrightarrow { \mathrm { x t } }$ 。 

2. 计算合理理论估值： $\displaystyle V _ { 0 } = \frac { D _ { 1 } } { r _ { s } - g } = \frac { 0 . 3 3 6 } { 1 0 \% - 5 \% } = 6 . 7 2$ 元。 

3. 计算理论市盈率（基于当期历史收益）： $\displaystyle \mathtt { P E } _ { \mathtt { H H i k } } = \frac { V _ { 0 } } { \mathtt { E P S } _ { 0 } } = \frac { 6 . 7 2 } { 0 . 4 0 } = 1 6 . 8$ 倍。 
...
案例 10 (2026  暨大：股权再投资回报率与公司价值). A 公司股权收益率 ROE = 18%, 再投资留 存比例 $b = 5 0 \%$ 。预计一年后每股净利润 $E _ { 1 } = 6 . 0 0 ~ \mathrm { \vec { \pi } }$ ，市场必要报酬率 $r _ { s } = 1 2 \%$ ，求该公司 股票价格的当期现值。 

\subsubsection{【答案/解析】}

1. 计算下一期预期派发的红利： $D _ { 1 } = E _ { 1 } \times ( 1 - b ) = 6 . 0 0 \times ( 1 - 0 . 5 0 ) = 3 . 0 0 \bar { \mathcal { I } }$ 元。 
...
$$
P _ {9} = \frac {D _ {1 0}}{r _ {s} - g} = \frac {8 . 0 0}{16 \% - 6 \%} = 80. 0 0
$$
...
案例12(2018 上财：反推永续阶段增长率与两阶段估值). A公司刚刚发放了每股 $D _ { 0 } = 0 . 5 0 ~ \mathrm { \vec { \pi } }$ 股利。预计未来 4 年内（高速成长第一阶段），红利年增长率为 $g _ { 1 } = 5 \%$ 。必要报酬率为 $r _ { s } = 1 5 \%$ 。 若已知第 4 年末分红之后股票的预期市场价格为 $P _ { 4 } = 1 0 . 0 0 ~ \hat { \pi }$ ，且从第 4 年起红利增长率调整 为 g2 并永续。求：（1）永续增长率 g2；（2）第五年末 $P _ { 5 }$ ；（3）当前股票估值 $P _ { 0 }$ 。 
...
案例 13 (2022 中财：再投资回报率与必要报酬率关系实证). 已知必要报酬率 $r _ { s } = 1 0 \%$ ，公司 每股永续盈利 E = 10.00 元，当期第 1 年末将每股利润 100% 留存用于投资项目（即 $D _ { 1 } = 0 )$ ）。 从第2 年起项目投产，公司恢复 100%分红（即 $D _ { 2 } = E _ { 2 } )$ ）。 
...
体积， DtP $\frac { D _ { t } } { P _ { t - 1 } }$ 为股利收益率， $\frac { P _ { t } - P _ { t - 1 } } { P _ { t - 1 } }$ 为资本利得率。 
...
$$
\sigma_ {p} ^ {2} = w _ {A} ^ {2} \sigma_ {A} ^ {2} + w _ {B} ^ {2} \sigma_ {B} ^ {2} + 2 w _ {A} w _ {B} \mathbf {C o v} (r _ {A}, r _ {B})
$$
...
$$
\rho_ {A B} = \mathbf {C o r r} (r _ {A}, r _ {B}) = \frac {\mathbf {C o v} (r _ {A} , r _ {B})}{\sigma_ {A} \sigma_ {B}} \in [ - 1, 1 ]
$$
...
当把资产配置推广到 N 个资产的场景时，组合方差由 N N 协方差矩阵的所有单元格之 和决定。 

组合方差的双重求和数学形式为： 

$$
\begin{aligned}
  \sigma_ {p} ^ {2} &= \sum_ {i = 1} ^ {N} \sum_ {j = 1} ^ {N} w _ {i} w _ {j} \mathbf {C o v} (r _ {i}, r _ {j}) \\[1.2ex]
  &= \sum_ {i = 1} ^ {N} w _ {i} ^ {2} \sigma_ {i} ^ {2} + 2 \sum_ {1 \leq i <   j \leq N} w _ {i} w _ {j} \mathbf {C o v} (r _ {i}, r _ {j})
\end{aligned}
$$
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{figure/convergence.png}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{figure/indifference.png}
\end{figure}
...
\begin{tabularx}{\textwidth}{lYY}
  ...
  0资金分仓买入风险资产与国债。无杠杆防御型分仓配置。 & 资金分仓买入风险资产与国债。 & 无杠杆防御型分仓配置。 \\
  w=1 & 满仓买入风险资产。 & 无负债杠杆。 \\
...
\textbf{\color{UnivColor} 【解析】}方差-协方差矩阵的对角线上是N 个资产各自的方差项。其余非对角线上的 $N ^ { 2 } - N$ 个 元素代表资产两两之间的协方差项。由于对角对称性 $( \mathbf { C o v } ( r _ { i } , r _ { j } ) = \mathbf { C o v } ( r _ { j } , r _ { i } ) )$ ），共有 $N ^ { 2 } - N$ 项（若问不重复的独立协方差项，则是 $\frac { N ^ { 2 } - N } { 2 }$ ，本题对应题意非对角线元素项数，故为 $N ^ { 2 } - N )$ ）。
