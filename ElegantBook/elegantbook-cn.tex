...
\textbf{\color{UnivColor} 【解析】}方差-协方差矩阵的对角线上是N 个资产各自的方差项。其余非对角线上的 $N ^ { 2 } - N$ 个 元素代表资产两两之间的协方差项。由于矩阵对称性（$\mathbf{Cov}(r_i, r_j) = \mathbf{Cov}(r_j, r_i)$），共有 $N^2 - N$ 项（若问不重复的独立协方差项，则是 $\frac{N^2 - N}{2}$ ，本题对应题意非对角线元素项数，故为 $N^2 - N$）。
