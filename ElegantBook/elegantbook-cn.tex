...
注. 案例背景：某公司股票当前 $\mathrm{EPS}_0 = 1.00$ 元，当前市价 $P_0 = 10.00$ 元。当期股利支付率 $d = 5 0 \%$ ，再 投资回报率 $\mathrm { R O E } = 1 0 \%$ 。必要报酬率 $r _ { s } = 1 0 . 2 5 \%$ 。若公司决定降低分红，将股利支付率下调至 30\%（即 留存比率 $b'$ 提高为70\%）： 
...
\textbf{\color{UnivColor} 【解析】}方差-协方差矩阵的对角线上是N 个资产各自的方差项。其余非对角线上的 $N ^ { 2 } - N$ 个 元素代表资产两两之间的协方差项。由于对角对称性（$\mathbf{Cov}(r_i, r_j) = \mathbf{Cov}(r_j, r_i)$），共有 $N^2 - N$ 项（若问不重复的独立协方差项，则是 $\frac{N^2 - N}{2}$ ，本题对应题意非对角线元素项数，故为 $N^2 - N$）。
