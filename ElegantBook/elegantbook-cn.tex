...
\begin{tabularx}{\textwidth}{llYY}
  ...
  高速成长永续期 & 固定增长模型(戈登增长模型) & 股利以恒定的增长率 $g$ 永续增长(必须满足必要报酬率$r_s > g$) & $P_0 = \dfrac{D_1}{r_s - g}$ \\
	初创与转型期 & 两阶段增长模型 & 增长率分段变化,前高后低。初期不规则高增长,后期转入永续平稳增长阶段。 & $\displaystyle P_0 = \sum_{t=1}^{n} \frac{D_t}{(1+r_s)^t} + \frac{V_n}{(1+r_s)^n}$\newline
	  其中 $D_1 = D_0(1+g)$，$V_n = \dfrac{D_{n+1}}{r_s - g_2}$ \\
      \bottomrule
\end{tabularx}
\end{table}
...
\begin{tabularx}{\textwidth}{llYY}
  ...
  有限分段折现 & $P_0 = \displaystyle\sum_{t=1}^{n} \frac{D_t}{(1+r_s)^t} + \frac{P_n}{(1+r_s)^n}$ & 两阶段不规则高增长 & 终值 $P_n$ 站在第 $n$ 年末,折现时务必使用 $n$ 次方 \\
